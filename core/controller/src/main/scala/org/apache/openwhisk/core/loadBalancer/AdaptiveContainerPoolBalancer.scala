...
class AdaptiveInvokerPoolMaintenance(var activeInvokers: ListBuffer[InvokerHealth],var inactiveInvokers: ListBuffer[InvokerHealth],logging: Logging){
  ...
  def upgradeInvoker(): Option[InvokerHealth] = {
    logging.info(this,s"<avs_debug> <AIPM> <upgradeInvoker> inactiveInvokers.size: ${inactiveInvokers.size} ")
  ...
  def dummySleep(): Future[Unit] = {
    ...
    logging.info(this,s"<avs_debug> <AIPM> <dummySleep> Slept for ${sleptTime} end: ${sleepEnd} begin: ${sleepStart} ")
    Future.successful(())
  }
  ...
  def makeInvokerActive(toActivateInvoker: InvokerHealth): Unit = {
    ...
    activeInvokers.foreach{
      curInvoker =>
      logging.info(this,s"<avs_debug> <AIPM> <makeInvokerActive:listInvokers> invoker: ${curInvoker.id.toInt} is currently ACTIVE.. ")
    }

    inactiveInvokers.foreach{
      curInvoker =>
      logging.info(this,s"<avs_debug> <AIPM> <makeInvokerActive:listInvokers> invoker: ${curInvoker.id.toInt} is currently INACTIVE.. ")
    }

    if(!activeInvokers.contains(toActivateInvoker)){
      //logging.info(this,s"<avs_debug> <AIPM> <makeInvokerActive> invoker: ${toActivateInvoker.id.toInt} is being made active.. and will sleep for threshold: ${makeActiveSleepInMS} ms ")
      ...
      val sleepEnd: Long = Instant.now.toEpochMilli
      val sleptTime: Long = sleepEnd - sleepStart
      logging.info(this,s"<avs_debug> <AIPM> <makeInvokerActive> invoker: ${toActivateInvoker.id.toInt} was made active and slept for ${sleptTime} end: ${sleepEnd} begin: ${sleepStart} ")*/
    }else{
  ...
  def downgradeInvoker(toDowngradeInvoker: ListBuffer[InvokerHealth]): Unit = {
    logging.info(this,s"<avs_debug> <AIPM> <downgradeInvoker> begin activeInvokers.size: ${activeInvokers.size} inactiveInvokers.size: ${inactiveInvokers.size}")
    toDowngradeInvoker.foreach{
      ...
      if(!(inactiveInvokers.exists( invoker => invoker == curInvoker))){
        logging.info(this,s"<avs_debug> <AIPM> <downgradeInvoker> apparently curInvoker: ${curInvoker.id.toInt} is not present in inactiveInvokers!")    
        inactiveInvokers+=curInvoker // this is OK..
      }
    }
    logging.info(this,s"<avs_debug> <AIPM> <downgradeInvoker> end activeInvokers.size: ${activeInvokers.size} inactiveInvokers.size: ${inactiveInvokers.size}")
  }
...
//extends CommonLoadBalancer(config, feedFactory,controllerInstance) // avs added loadFeedFactory
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    val chosen = if (invokersToUse.nonEmpty) {
      ...
      val invoker: Option[(InvokerInstanceId, Boolean)] = AdaptiveContainerPoolBalancer.schedule(
...
// avs --begin
    ...
    if( (curInstant-lastUpdatedTime ) > scanIntervalInMS){
        ...
        logging.info(this,s"<avs_debug> <AdaptiveContainerPoolBalancer> <publish> During schedule of activation ${msg.activationId} for '${msg.action.asString}' ($actionType), doing background maintenance") // avs
...
// avs --begin
  //@tailrec
    def schedule(
      ...
      logging.info(this,s"<avs_debug> <schedule> 0.0 ") // avs
      val invoker: Option[InvokerInstanceId] = getUsedInvokerForAction(actionName) //curInvokerPoolMaintenance.getExistingContainer(actionName)
      invoker match {
        ...
        case None =>
          ...
          newInvokerForAction match{ 
            ...
            case None =>
              ...
              val newActiveInvoker: Option[InvokerHealth] = curInvokerPoolMaintenance.upgradeInvoker() //actionName,activeInvokers) 
              newActiveInvoker match{ 
                case Some (newChosenActiveInvoker) => 
                  logging.info(this,s"<avs_debug> <schedule> 1.3, got an invoker: ${newChosenActiveInvoker.id.toInt} from upgradeInvoker!! ") // avs
                  if(checkInvokerCapacity(newChosenActiveInvoker,actionName)){
                    return Some(newChosenActiveInvoker.id,true)
                  }
                case None =>
                  logging.info(this,s"<avs_debug> <schedule> 1.31, DID-NOT get an invoker from upgradeInvoker!! ") // avs
                  // ok, random selection it is....
              }
          }
      }
...
// avs --begin
    if(!aipmInit){
      ...
      logging.info(this,s"<avs_debug>, DONE populating, curInvokerPoolMaintenance object! ")
      aipmInit = true      
    }
// avs --end       
  }
